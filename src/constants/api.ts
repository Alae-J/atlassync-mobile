 ...
 *
 * - In development, Expo exposes the dev-server LAN IP via `expoConfig.hostUri`.
 *   We reuse that host with port 8080 so a real iPhone on the same Wi-Fi can reach
 *   the auth-service running on the developer's laptop without any manual config.
 * - When `EXPO_PUBLIC_API_BASE_URL` is provided (build-time), it overrides everything.
 * - As a last fallback we use `localhost:8080`, which only works for the iOS
